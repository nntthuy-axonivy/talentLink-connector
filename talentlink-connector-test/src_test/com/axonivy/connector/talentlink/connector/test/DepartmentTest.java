...
import com.lumessetalentlink.shared.apiproxy.tlk.rest.client.DepartmentFullDto;
...
private static final BpmElement getDepartmentTestee = BpmProcess.path("rest/Department").elementName("getDepartment()");
	...
	public void testGetById(BpmClient bpmClient, AppFixture fixture) {
		ExecutionResult result = bpmClient.start().subProcess(getTestee()).withParam("departmentId", 352L).execute();
		List<DepartmentFullDto> entity = result.subResult().firstParam();
